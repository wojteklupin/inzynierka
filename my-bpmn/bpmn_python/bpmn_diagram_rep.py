...
class BpmnDiagramGraph(object):
    ...
    * plane_attributes - dictionary that contains BPMN plane element attributes.
    ...
    def add_boundary_event_to_task(self, process_id, task_id, type, interrupting=True, boundary_event_name=""):
        """
        Adds a boundary event element to a task.
        User-defined attributes:

        :param process_id: string object. ID of parent process,
        :param task_id: string object. ID of task to attach to.
        :param type: string object. One of: "message", "timer", ...,
        :param interrupting: bool value. True if interrupting, else non-interrupting,
        :param boundary_event_name: string object,
        :return: a tuple, where first value is task ID, second a reference to created object.
        """
        if type in ["message", "timer", "conditional", "signal", "escalation"]:
            boundary_id, boundary = self.add_flow_node_to_diagram(process_id, consts.Consts.boundary_event, boundary_event_name, None)
            self.diagram_graph._node[boundary_id][consts.Consts.attached_to_ref] = task_id
            self.diagram_graph._node[boundary_id][consts.Consts.parallel_multiple] = "false"
            self.diagram_graph._node[boundary_id][consts.Consts.cancel_activity] = str(interrupting).lower()
            self.diagram_graph._node[boundary_id][consts.Consts.width] = "40"
            self.diagram_graph._node[boundary_id][consts.Consts.height] = "40"
            ...
            event_def_list = []
            event_def_list.append(BpmnDiagramGraph.add_event_definition_element(type, event_definitions))
            self.diagram_graph._node[boundary_id][consts.Consts.event_definitions] = event_def_list
            return boundary_id, boundary
        else:
            raise bpmn_exception.BpmnPythonError("Invalid value passed as type parameter")

    def add_intermediate_event_to_diagram(self, process_id, type, subtype, boundary_event_name=""):
        """
        Adds a boundary event element to diagram.
        User-defined attributes:

        :param process_id: string object. ID of parent process,
        :param type: string object. "intermediateCatchEvent" or "intermediateThrowEvent",
        :param subtype: string object. One of: "message", "timer", ...,
        :param boundary_event_name: string object,
        :return: a tuple, where first value is task ID, second a reference to created object.
        """
        if type in [consts.Consts.intermediate_catch_event, consts.Consts.intermediate_throw_event] and \
        subtype in ["message", "timer", "conditional", "signal", "escalation"]:
            boundary_id, boundary = self.add_flow_node_to_diagram(process_id, type, boundary_event_name, None)
            self.diagram_graph._node[boundary_id][consts.Consts.parallel_multiple] = "false"
            ...
            event_def_list = []
            event_def_list.append(BpmnDiagramGraph.add_event_definition_element(subtype, event_definitions))
            self.diagram_graph._node[boundary_id][consts.Consts.event_definitions] = event_def_list
            return boundary_id, boundary
        else:
            raise bpmn_exception.BpmnPythonError("Invalid value passed as type or subtype parameter")
    ...
    def add_start_event_to_diagram(self, process_id, start_event_name="", start_event_definition=None,
        ...
        self.diagram_graph._node[start_event_id][consts.Consts.width] = "50"
        self.diagram_graph._node[start_event_id][consts.Consts.height] = "50"
    ...
    def add_end_event_to_diagram(self, process_id, end_event_name="", end_event_definition=None, node_id=None):
        ...
        self.diagram_graph._node[end_event_id][consts.Consts.width] = "50"
        self.diagram_graph._node[end_event_id][consts.Consts.height] = "50"
    ...
    def add_gateway_to_diagram(self, process_id, gateway_type, gateway_name="", gateway_direction="Unspecified",
        ...
        self.diagram_graph._node[gateway_id][consts.Consts.width] = "50"
        self.diagram_graph._node[gateway_id][consts.Consts.height] = "50"
    ...
    def add_exclusive_gateway_to_diagram(self, process_id, gateway_name="", gateway_direction="Unspecified",
        ...
        self.diagram_graph._node[exclusive_gateway_id][consts.Consts.width] = "50"
        self.diagram_graph._node[exclusive_gateway_id][consts.Consts.height] = "50"
