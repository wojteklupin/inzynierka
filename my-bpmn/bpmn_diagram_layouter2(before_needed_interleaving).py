...
def grid_layout(bpmn_graph, sorted_nodes_with_classification):
    ...
    :return:
    """
    for node in sorted_nodes_with_classification:
        node['node'][1]['incoming']
            ...
            if num_of_successors % 2 != 0:
                ...
                for index in range(0, centre):
                    ...
                    (grid, last_row, last_col) = place_element_in_grid(successor_node, grid, last_row, last_col,
                                                                    bpmn_graph,nodes_with_classification, all_nodes_with_classification, current_element_row + ((index + 1) * consts.Consts.grid_column_width))
                    nodes_with_classification.remove(successor_node)

                successor_node = successor_node_list[centre]
                (grid, last_row, last_col) = place_element_in_grid(successor_node, grid, last_row, last_col,
                                                                bpmn_graph,nodes_with_classification, all_nodes_with_classification, current_element_row)
                nodes_with_classification.remove(successor_node)
                for index in range(centre + 1, num_of_successors):
                    # place element above split
                    successor_node = successor_node_list[index]
                    (grid, last_row, last_col) = place_element_in_grid(successor_node, grid, last_row, last_col,
                                                                    bpmn_graph,nodes_with_classification, all_nodes_with_classification, current_element_row - ((index - centre) * consts.Consts.grid_column_width))
                    nodes_with_classification.remove(successor_node)
                ...
                for index in range(centre, num_of_successors):
                    ...
                    successor_node = successor_node_list[index]
                    (grid, last_row, last_col) = place_element_in_grid(successor_node, grid, last_row, last_col,
                                                                    bpmn_graph,nodes_with_classification, all_nodes_with_classification, current_element_row - ((index - centre + 1) * consts.Consts.grid_column_width))
                    nodes_with_classification.remove(successor_node)
